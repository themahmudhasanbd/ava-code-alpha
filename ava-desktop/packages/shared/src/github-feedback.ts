export const GITHUB_REPO = "themahmudhasanbd/ava-code-alpha";
...
export function formatFeedbackEnvironment(info: FeedbackIssueContext): string {
  ...
  return `AvA Code Alpha ${info.version} · ${info.platform} ${info.arch} · protocol ${info.protocolVersion} · host ${host}`;
}
