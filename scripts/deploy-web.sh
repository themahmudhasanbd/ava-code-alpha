...
COMMIT_MSG="${1:-feat(web): auto-deploy AvA web portal [$(date +'%Y-%m-%d %H:%M:%S')]}"
...
if [[ -n $(git status -s) ]]; then
    echo -e "${YELLOW}Changes detected. Staging and committing...${NC}"
    git add -A
    git commit -m "$COMMIT_MSG"
    echo -e "${GREEN}✓ Committed: ${COMMIT_MSG}${NC}"
else
    echo -e "${GREEN}✓ Working tree clean. No new changes to commit.${NC}"
fi

echo -e "${BLUE}Pushing branch '${BRANCH}' to origin...${NC}"
git push origin "$BRANCH" || {
    echo -e "${YELLOW}Warning: Git push failed or remote was busy, continuing build...${NC}"
}
...
echo -e "${CYAN}Running Flutter Web compilation...${NC}"
"$FLUTTER_BIN" build web --release --no-wasm-dry-run

if [ ! -d "$MOBILE_DIR/build/web" ]; then
    echo -e "${RED}Error: Build directory $MOBILE_DIR/build/web does not exist!${NC}"
    exit 1
fi
...
echo -e "${GREEN}✓ Web files synced and permissions updated.${NC}\n"
...
if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 301 ] || [ "$HTTP_CODE" -eq 302 ]; then
    echo -e "${GREEN}✓ Health check passed! HTTP $HTTP_CODE from https://${DOMAIN}/${NC}"
else
    echo -e "${YELLOW}Warning: HTTP Status $HTTP_CODE received from https://${DOMAIN}/${NC}"
fi
