...
class CustomTextEditState extends State<CustomTextEdit> with TextInputClient {
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    final currentText = _currentEditingState.text;

    if (currentText.startsWith(initText)) {
      if (currentText.length > initText.length) {
        final textDelta = currentText.substring(initText.length);
        widget.onInsert(textDelta);
      } else if (currentText.length < initText.length) {
        widget.onDelete();
      }
    } else {
      // IME replaced the text (e.g., multilingual IME / Bangla composing commit)
      if (currentText.isNotEmpty) {
        final cleanText = currentText.trimLeft();
        if (cleanText.isNotEmpty) {
          widget.onInsert(cleanText);
        } else {
          widget.onDelete();
        }
      } else {
