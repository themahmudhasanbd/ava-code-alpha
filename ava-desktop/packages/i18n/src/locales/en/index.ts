export const en = {
  app: {
    shellName: "AvA Code",
    tagline: "Autonomous AI Coding Partner — v Alpha",
    starting: "Starting AvA Code…",
    loadingView: "Loading view…",
    uiCrashed: "Something went wrong with the interface",
  },
  startup: {
    slowTitle: "Still starting…",
    slowBody: "AvA Code is taking longer than usual to reach your local service. It may still finish on its own — you can wait, or collect the logs first.",
    stalledTitle: "AvA Code couldn't finish starting",
  ...
  tray: {
    ...
    open: "Open AvA Code",
    quit: "Quit AvA Code",
    askTitle: "Keep AvA Code running in the background?",
    askBody:
      "When you close the window, AvA Code can keep running in the system tray so nothing is lost. You can change this any time in Settings.",
    closeToTray: "Close to tray",
    confirmQuitTitle: "Quit AvA Code?",
    confirmQuitBody:
  ...
  chat: {
    ...
    placeholder: "Ask AvA Code to help with anything",
...
sklm: {
    ...
    pluginProviderKeyHint:
      "Stored in AvA Code and used by the runtime. The plugin never receives it.",
  ...
  scheduled: {
    description: "Run recurring agent tasks while AvA Code is open.",
    edit: "Edit task",
    ...
    localTimeHint: "Uses this computer’s local timezone. Keep AvA Code running; missed runs are skipped. Automatic runs ask before restricted actions.",
...
importConfirm: "Imported extensions run inside the agent process with the same access as the agent's own tools. Declared skills can add instructions to the agent, and declared dependencies are installed with npm (install scripts disabled). Continue?",
    ...
    permissionHelp: {
      ...
      "provider.register":
        "Adds the providers this plugin defines to Settings' provider list. The plugin supplies the endpoint and models; your API key stays in AvA Code.",
      "desktop.control":
        "Lets the plugin invoke the reviewed AvA Code control catalog. Destructive operations still require confirm=true; the MCP bearer token is never exposed.",
      ...
      "keyboard.globalShortcut": "Registers system-wide keyboard shortcuts that trigger this plugin's own commands while AvA Code is not focused.",
  ...
  status: {
    ...
    dbSchemaTooNew:
      "This AvA Code is older than your local data (data schema {{found}}, this build supports {{supported}}). Install the newer AvA Code that last opened this data, or a later version.",
    archMismatch:
