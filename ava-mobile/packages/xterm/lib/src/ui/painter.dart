...
class TerminalPainter {
  ...
  bool _isIndicOrCombining(int code) {
    return (code >= 0x0980 && code <= 0x09FF) || // Bengali
  ...
  void paintLine(
    ...
    for (var i = 0; i < line.length; i++) {
      ...
      if (_isIndicOrCombining(charCode)) {
        // Collect entire Indic/Bengali cluster to allow HarfBuzz ligature shaping
        final runChars = <int>[];
        var j = i;
        while (j < line.length) {
          final c = line.getCodePoint(j);
          if (c == 0 || !_isIndicOrCombining(c)) break;
          runChars.add(c);
          line.getCellData(j, cellData);
          paintCellBackground(canvas, offset.translate(j * cellWidth, 0), cellData);
          j++;
        }

        if (runChars.isNotEmpty) {
          final text = String.fromCharCodes(runChars);
          line.getCellData(i, cellData);
          ...
          canvas.drawParagraph(p, cellOffset);
          p.dispose();
